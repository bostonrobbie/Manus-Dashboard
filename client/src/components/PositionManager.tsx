...
import { AlertTriangle, CheckCircle, RefreshCw, XCircle, History, Trash2 } from "lucide-react";
...
export function PositionManager() {
  ...
  const handleResolve = (discrepancyId: number, action: string) => {
    resolveDiscrepancyMutation.mutate({
      discrepancyId,
      action: action as "sync_from_broker" | "force_close" | "ignore" | "manual_fix",
    });
  };
                  ...
                  <TableBody>
                    {positions.map((pos: { id: number; strategySymbol: string; direction: string; quantity: number; entryPrice: number; entryTime: string | Date; status: string }) => (
                      <TableRow key={pos.id}>
                        ...
                        <TableCell>
                          <Dialog open={isForceCloseOpen && selectedPosition === pos.id} onOpenChange={(open) => {
                            ...
                            <DialogContent>
                              <DialogHeader>
                                <DialogTitle>Force Close Position</DialogTitle>
                                <DialogDescription>
                                  This will mark the position as closed without an exit signal.
                                  Use this to fix database state when the position was closed manually in the broker.
                                </DialogDescription>
                              </DialogHeader>
                              <div className="space-y-4 py-4">
                                <div className="space-y-2">
                                  ...
                                  <div className="text-sm text-muted-foreground">
                                    {pos.strategySymbol} - {pos.direction.toUpperCase()} {pos.quantity} @ {formatPrice(pos.entryPrice)}
                                  </div>
                                </div>
                                <div className="space-y-2">
                                  <Label htmlFor="reason">Reason for Force Close</Label>
                                  <Textarea
                                    id="reason"
                              ...
                              <DialogFooter>
                                <Button variant="outline" onClick={() => setIsForceCloseOpen(false)}>
                                  Cancel
                                </Button>
                                <Button 
                                  variant="destructive" 
        ...
        <TabsContent value="discrepancies" className="space-y-4">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button variant="outline" size="sm" onClick={() => refetchDiscrepancies()}>
                <RefreshCw className="h-4 w-4 mr-2" />
                Refresh
              </Button>
            </CardHeader>
                  ...
                  <TableBody>
                    {discrepancies.map((disc: { id: number; symbol: string; discrepancyType: string; dbDirection?: string; dbQuantity?: number; brokerDirection?: string; brokerQuantity?: number; discrepancyDetails?: string }) => (
                      <TableRow key={disc.id}>
                        ...
                        <TableCell>
                          <Badge variant={
                            disc.discrepancyType === "missing_in_db" ? "destructive" :
                            disc.discrepancyType === "missing_in_broker" ? "secondary" :
                            "outline"
                          }>
                        ...
                        <TableCell>
                          {disc.dbDirection ? (
                            <span>{disc.dbDirection} {disc.dbQuantity}</span>
                          ) : (
                        ...
                        <TableCell>
                          {disc.brokerDirection ? (
                            <span>{disc.brokerDirection} {disc.brokerQuantity}</span>
                          ) : (
                            <span className="text-muted-foreground">-</span>
                          )}
                        </TableCell>
                        ...
                        <TableCell>
                          <Select onValueChange={(value) => handleResolve(disc.id, value)}>
                            <SelectTrigger className="w-32">
                              <SelectValue placeholder="Resolve" />
                            </SelectTrigger>
                            <SelectContent>
                              <SelectItem value="sync_from_broker">Sync from Broker</SelectItem>
                              <SelectItem value="force_close">Force Close</SelectItem>
                              <SelectItem value="ignore">Ignore</SelectItem>
                              <SelectItem value="manual_fix">Mark Fixed</SelectItem>
                            </SelectContent>
                          </Select>
                        </TableCell>
                      </TableRow>
                  ...
                  <TableBody>
                    {adjustmentHistory.map((adj: { id: number; createdAt: string | Date; strategySymbol: string; adjustmentType: string; beforeDirection?: string; beforeQuantity?: number; afterDirection?: string; afterQuantity?: number; reason: string; adjustedBy: string }) => (
                      <TableRow key={adj.id}>
                        ...
                        <TableCell>
                          {adj.beforeDirection ? (
                            <span>{adj.beforeDirection} {adj.beforeQuantity}</span>
                          ) : (
                            <span className="text-muted-foreground">-</span>
                          )}
                        </TableCell>
                        <TableCell>
                          {adj.afterDirection ? (
                            <span>{adj.afterDirection} {adj.afterQuantity}</span>
                          ) : (
