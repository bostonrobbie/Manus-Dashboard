...
function runMonteCarloSimulation(
  ...
  const avgDrawdown = maxDrawdowns.reduce((sum, d) => sum + d, 0) / numSimulations;
  ...
  const expectedAnnualReturn = yearsSimulated > 0 ? Math.pow(1 + totalReturn, 1 / yearsSimulated) - 1 : 0;
...
export default function MonteCarloSimulation({ trades, startingCapital }: MonteCarloSimulationProps) {
  ...
  const getAssessment = () => {
    ...
    if (probProfit >= 0.9 && probRuin < 0.05 && maxDrawdown < 0.2) {
      return { 
        status: 'excellent', 
        message: 'Excellent risk profile. This strategy shows strong probability of profit with manageable risk.',
        icon: CheckCircle2,
      ...
      return { 
        status: 'good', 
        message: 'Good risk profile. Consider the recommended position sizing to optimize returns.',
        icon: CheckCircle2,
        color: 'text-green-500'
      };
      ...
      return { 
        status: 'moderate', 
        message: 'Moderate risk. Use conservative position sizing and ensure adequate capital reserves.',
        icon: AlertTriangle,
      ...
      return { 
        status: 'high-risk', 
        message: 'High risk profile. Consider reducing position size or improving strategy edge before trading live.',
        icon: XCircle,
        color: 'text-red-500'
      };
    }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {numSimulations.toLocaleString()} simulations of {trades.length.toLocaleString()} trades to estimate future outcomes
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        {simulationResult && (
          <div className={`p-4 rounded-lg border-2 ${
            ...
            <div className="flex items-start gap-3">
              {assessment.icon && <assessment.icon className={`h-6 w-6 ${assessment.color} flex-shrink-0 mt-0.5`} />}
              <div>
                <h3 className={`font-semibold ${assessment.color}`}>
                  {assessment.status === 'excellent' ? 'Excellent Risk Profile' :
                   assessment.status === 'good' ? 'Good Risk Profile' :
                   assessment.status === 'moderate' ? 'Moderate Risk' : 'High Risk Warning'}
                </h3>
                <p className="text-sm text-muted-foreground mt-1">{assessment.message}</p>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-blue-500/10 rounded-lg p-4 border border-blue-500/20">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-medium text-blue-500">RECOMMENDED POSITION SIZE</span>
              </div>
            ...
            <div className="bg-purple-500/10 rounded-lg p-4 border border-purple-500/20">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-medium text-purple-500">MINIMUM ACCOUNT SIZE</span>
              </div>
              <div className="text-2xl font-bold">
                ${simulationResult.stats.recommendedCapital.toLocaleString(undefined, { maximumFractionDigits: 0 })}
              </div>
              <p className="text-xs text-muted-foreground mt-1">
            ...
            <div className="bg-emerald-500/10 rounded-lg p-4 border border-emerald-500/20">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-medium text-emerald-500">95% CONFIDENCE OUTCOME</span>
              </div>
              <div className="text-2xl font-bold">
                {simulationResult.stats.probProfit >= 0.95 ? (
                  <span className="text-green-500">Profitable</span>
                ) : simulationResult.stats.probProfit >= 0.80 ? (
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {(simulationResult.stats.probProfit * 100).toFixed(1)}% probability of profit
              </p>
            </div>
          </div>
        )}
        ...
        {simulationResult && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-muted/30 rounded-lg p-3 border">
              <div className="text-xs text-muted-foreground mb-1">Expected Outcome (Median)</div>
              <div className="text-lg font-bold">
                ${simulationResult.stats.medianFinal.toLocaleString(undefined, { maximumFractionDigits: 0 })}
              </div>
              <div className="text-xs text-muted-foreground">
                {((simulationResult.stats.medianFinal - startingCapital) / startingCapital * 100).toFixed(1)}% return
              </div>
            </div>
            ...
            <div className="bg-green-500/10 rounded-lg p-3 border border-green-500/20">
              ...
              <div className="text-xs text-green-600">
                +{((simulationResult.stats.bestCase - startingCapital) / startingCapital * 100).toFixed(1)}%
              </div>
            </div>
            ...
            <div className="bg-red-500/10 rounded-lg p-3 border border-red-500/20">
              ...
              <div className="text-xs text-red-600">
                {((simulationResult.stats.worstCase - startingCapital) / startingCapital * 100).toFixed(1)}%
              </div>
            </div>
            
            <div className="bg-orange-500/10 rounded-lg p-3 border border-orange-500/20">
              <div className="text-xs text-muted-foreground mb-1">Max Drawdown (95th %ile)</div>
              <div className="text-lg font-bold text-orange-600">
                {(simulationResult.stats.maxDrawdown * 100).toFixed(1)}%
              </div>
              <div className="text-xs text-orange-600">
                ${(simulationResult.stats.maxDrawdown * startingCapital).toLocaleString(undefined, { maximumFractionDigits: 0 })}
              </div>
            </div>
            ...
            <div className="bg-muted/30 rounded-lg p-3 border">
              ...
              <div className={`text-lg font-bold ${simulationResult.stats.probRuin < 0.05 ? 'text-green-600' : simulationResult.stats.probRuin < 0.15 ? 'text-yellow-600' : 'text-red-600'}`}>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={sampledData} margin={{ top: 10, right: 10, left: 10, bottom: 10 }}>
                ...
                <XAxis
                  ...
                  label={{ value: 'Trade Number', position: 'insideBottom', offset: -5, fill: '#ffffff', fontSize: 11 }}
                />
                <YAxis
                  tick={{ fontSize: 10, fill: '#ffffff' }}
                  tickLine={{ stroke: 'rgba(255,255,255,0.3)' }}
                  ...
                  width={55}
                  label={{ value: 'Portfolio Value', angle: -90, position: 'insideLeft', fill: '#ffffff', fontSize: 11, dx: -5 }}
                />
                <Tooltip
                  contentStyle={{
        ...
        <div className="text-xs text-muted-foreground bg-muted/20 rounded-lg p-4">
          <p className="font-semibold mb-2 text-sm">How to Use These Results:</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <p className="font-medium text-foreground">Position Sizing:</p>
              <p>Risk no more than the recommended percentage per trade to survive drawdowns.</p>
            </div>
            <div>
              <p className="font-medium text-foreground">Account Size:</p>
              <p>Ensure your account exceeds the minimum to handle worst-case scenarios.</p>
            </div>
            <div>
              <p className="font-medium text-foreground">Confidence Level:</p>
              <p>95% confidence means only 5% of simulations performed worse than shown.</p>
            </div>
            <div>
              <p className="font-medium text-foreground">Risk of Ruin:</p>
              <p>Below 5% is excellent, 5-15% is acceptable, above 15% requires caution.</p>
            </div>
          </div>
        </div>
      </CardContent>
