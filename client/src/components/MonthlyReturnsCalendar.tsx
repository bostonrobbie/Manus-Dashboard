...
export function MonthlyReturnsCalendar({ monthlyReturns }: MonthlyReturnsCalendarProps) {
  ...
  const getColor = (returnPct: number): string => {
    ...
    if (returnPct > 0) return 'bg-green-300 text-gray-900';
    if (returnPct === 0) return 'bg-gray-400 text-gray-900';
    if (returnPct > -2) return 'bg-red-300 text-gray-900';
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Monthly Returns Calendar</CardTitle>
        <CardDescription>
          Monthly performance heatmap - green indicates positive returns, red indicates negative
        </CardDescription>
      </CardHeader>
      <CardContent>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr>
                ...
                {monthNames.map(month => (
                  <th key={month} className="text-center p-2 font-medium">{month}</th>
                ))}
            ...
            <tbody>
              {years.map(year => {
                ...
                const yearTotal = yearData.reduce((sum, mr) => sum + mr.return, 0);
