...
export default function Onboarding() {
  ...
  const steps: { id: Step; title: string; description: string }[] = [
    { id: "welcome", title: "Welcome", description: "Get started with Intraday Strategies" },
    { id: "strategies", title: "Choose Strategies", description: "Select strategies to follow" },
    { id: "preferences", title: "Preferences", description: "Set up your notifications" },
  ...
  const handleNext = async () => {
    const stepOrder: Step[] = ["welcome", "strategies", "preferences", "complete"];
          ...
          {currentStep === "welcome" && (
            <>
              <CardHeader className="text-center pb-2">
                ...
                <CardTitle className="text-2xl">Welcome, {user.name || "Trader"}!</CardTitle>
                  ...
                  <div className="flex items-start gap-4 p-4 rounded-lg bg-muted/50">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        Access algorithmic trading strategies with 14+ years of backtested data
                      </p>
                    </div>
                  </div>
                  <div className="flex items-start gap-4 p-4 rounded-lg bg-muted/50">
          ...
          {currentStep === "strategies" && (
            <>
              <CardHeader className="text-center pb-2">
                ...
                <CardTitle className="text-2xl">Choose Your Strategies</CardTitle>
                <CardDescription className="text-base">
                  Select the strategies you want to follow (you can change this later)
                </CardDescription>
              </CardHeader>
              <CardContent>
                ...
                <div className="p-4 rounded-lg border border-amber-500/50 bg-amber-500/5">
                  <div className="flex items-start gap-3">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        setPreferences((p) => ({ ...p, riskAcknowledged: !!checked }))
                      }
                    />
                    <div>
                      <Label htmlFor="risk-acknowledgment" className="font-medium text-amber-600">
                        Risk Acknowledgment
                      </Label>
                      <p className="text-sm text-muted-foreground mt-1">
                        I understand that trading involves substantial risk of loss and is not suitable for all investors. 
                        Past performance is not indicative of future results. I will only trade with capital I can afford to lose.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <CardContent className="space-y-6">
                <div className="grid gap-4">
                  ...
                  {selectedStrategies.length > 0 && (
                    <div className="flex items-center gap-4 p-4 rounded-lg bg-muted/50">
                      <Check className="h-5 w-5 text-green-500" />
                      <span>{selectedStrategies.length} strategies subscribed</span>
                    </div>
                  )}
                  <div className="flex items-center gap-4 p-4 rounded-lg bg-muted/50">
                    <Check className="h-5 w-5 text-green-500" />
                ...
                <div className="text-center text-sm text-muted-foreground">
                  <p>You can always update your strategies and preferences from your dashboard.</p>
                </div>
              </CardContent>
            </>
          )}
