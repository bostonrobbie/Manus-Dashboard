...
- **server/** – Express server exposing a `/trpc` endpoint and a `/health` route. Business logic and analytics live in `server/engine` and `server/portfolio-engine.ts` and are consumed by tRPC routers in `server/routers`.
