...
- **Migrations**: Stored in `drizzle/migrations`. Configure credentials in `.env` and run `pnpm --filter drizzle run migrate` to execute the Drizzle CLI against `DATABASE_URL`.
- **Config**: `drizzle.config.cjs` sets the schema entry point and migration output directory.
