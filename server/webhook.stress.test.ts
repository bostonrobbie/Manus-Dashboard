...
describe('Webhook Stress Tests', () => {
  ...
  describe('Concurrent Request Handling', () => {
    ...
    it('should correctly count requests across concurrent calls', async () => {
      ...
      const promises = Array(60).fill(null).map(() => 
        Promise.resolve(checkRateLimit(ip, config))
      );
  ...
  describe('Large Payload Handling', () => {
    ...
    it('should handle multiple_accounts with many entries', () => {
      const payload = {
        symbol: 'ESTrend',
        multiple_accounts: Array(50).fill(null).map((_, i) => ({
          account: `account_${i}`,
          quantity: i + 1,
        })),
      };
    ...
    it('should reject multiple_accounts exceeding 100 entries', () => {
      const payload = {
        symbol: 'ESTrend',
        multiple_accounts: Array(150).fill(null).map((_, i) => ({
          account: `account_${i}`,
          quantity: 1,
        })),
      };
  ...
  describe('Error Recovery', () => {
    ...
    it('should not leak state between requests', () => {
      ...
      // Request from IP A
      const resultA1 = checkRateLimit('ip-a', { windowMs: 60000, maxRequests: 2 });
      const resultA2 = checkRateLimit('ip-a', { windowMs: 60000, maxRequests: 2 });
      
      // Request from IP B should have fresh state
      const resultB = checkRateLimit('ip-b', { windowMs: 60000, maxRequests: 2 });
...
describe('Webhook Service Validation', () => {
  describe('Symbol Mapping', () => {
    it('should map all known TradingView symbols', () => {
      const mappings = [
        ['ES', 'ESTrend'],
        ['ES1!', 'ESTrend'],
        ['NQ', 'NQTrend'],
    ...
    it('should return original for unknown symbols', () => {
  ...
  describe('Timestamp Parsing', () => {
    ...
    it('should throw for invalid formats', () => {
      expect(() => parseTimestamp('not-a-date')).toThrow(WebhookValidationError);
  ...
  describe('Payload Validation', () => {
    ...
    it('should normalize action values', () => {
      expect(validatePayload({ symbol: 'ES', data: 'buy', price: 100 }).action).toBe('buy');
      expect(validatePayload({ symbol: 'ES', data: 'BUY', price: 100 }).action).toBe('buy');
      expect(validatePayload({ symbol: 'ES', data: 'long', price: 100 }).action).toBe('buy');
      expect(validatePayload({ symbol: 'ES', data: 'sell', price: 100 }).action).toBe('sell');
      expect(validatePayload({ symbol: 'ES', data: 'short', price: 100 }).action).toBe('sell');
      expect(validatePayload({ symbol: 'ES', data: 'exit', price: 100 }).action).toBe('exit');
    });

    it('should throw for missing symbol', () => {
      expect(() => validatePayload({ data: 'buy', price: 100 })).toThrow(WebhookValidationError);
    });

    it('should throw for missing action', () => {
      expect(() => validatePayload({ symbol: 'ES', price: 100 })).toThrow(WebhookValidationError);
    });

    it('should throw for missing price', () => {
      expect(() => validatePayload({ symbol: 'ES', data: 'buy' })).toThrow(WebhookValidationError);
    });

    it('should throw for invalid action', () => {
      expect(() => validatePayload({ symbol: 'ES', data: 'invalid', price: 100 })).toThrow(WebhookValidationError);
    });

    it('should handle string prices', () => {
      const result = validatePayload({ symbol: 'ES', data: 'buy', price: '4500.50' });
      expect(result.price).toBe(4500.50);
    });
