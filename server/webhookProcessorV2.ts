...
export async function processWebhookRobust(
  ...
  try {
    ...
    try {
      ...
      parsedPreview = {
        strategySymbol: mapSymbolToStrategy(
          String(p.symbol || p.strategy || "")
        ),
...
async function processEntrySignalWithTransaction(
  ...
  return await withTransaction(async connection => {
    ...
    // Send notification (async, non-blocking) - skip for test webhooks
    if (!isTestWebhook) {
      notifyOwnerAsync({
...
export async function replayWebhook(walId: string): Promise<WebhookResult> {
  ...
  return await processWebhookRobust(
    payload,
    entry.sourceIp || undefined,
