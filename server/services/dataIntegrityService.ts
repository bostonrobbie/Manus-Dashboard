...
export async function validateDataIntegrity(): Promise<ValidationResult> {
  ...
  const orphanedPositions = await db.select({
    id: openPositions.id,
  ...
  .where(and(
    eq(openPositions.status, 'closed'),
    isNotNull(openPositions.tradeId)
  ));
  ...
  const exitWebhooksWithoutTrade = await db.select({
    id: webhookLogs.id,
  ...
  const stalePositions = await db.select({
    id: openPositions.id,
    strategySymbol: openPositions.strategySymbol,
...
export async function quickHealthCheck(): Promise<{
  ...
  const checks: { name: string; status: 'pass' | 'fail' | 'warn'; message: string; value?: number }[] = [];
  ...
  if (failureRate > 20) {
    ...
    checks.push({
      name: 'Webhook Success Rate',
      status: 'pass',
      message: total > 0 ? `${(100 - failureRate).toFixed(1)}% success rate (last hour)` : 'No webhooks in last hour',
      value: 100 - failureRate,
    });
  }
  ...
  if (avgLatency > 1000) {
    ...
    checks.push({
      name: 'Processing Latency',
      status: 'pass',
      message: avgLatency > 0 ? `Avg: ${avgLatency.toFixed(0)}ms, Max: ${maxLatency.toFixed(0)}ms` : 'No recent data',
      value: avgLatency,
    });
  }
...
export async function getReconciliationReport(): Promise<{
  ...
  const positionTradeIds = new Set(allClosedPositions.map(p => p.tradeId).filter(Boolean));
