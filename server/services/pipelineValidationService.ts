...
export function validateCSVImport(
  ...
  for (let i = 0; i < tradesToImport.length; i++) {
    ...
    if (seenTrades.has(tradeKey)) {
      duplicates++;
      warnings.push(`Row ${tradeIndex}: Duplicate trade detected (same entry/exit date, price, direction)`);
      continue;
    }
    ...
    if (!validation.isValid) {
      invalidTrades++;
      validation.errors.forEach(err => errors.push(`Row ${tradeIndex}: ${err}`));
    } else {
      validTrades++;
    }

    validation.warnings.forEach(warn => warnings.push(`Row ${tradeIndex}: ${warn}`));
    ...
    if (trade.entryPrice < 100 || trade.entryPrice > 5000000) {
      warnings.push(`Row ${tradeIndex}: Entry price ${trade.entryPrice} seems unusual`);
    }

    // Check P&L consistency
    const expectedPnlDirection = trade.direction === 'Long'
      ? (trade.exitPrice > trade.entryPrice ? 1 : -1)
      : (trade.exitPrice < trade.entryPrice ? 1 : -1);
    
    if (trade.pnl !== 0 && Math.sign(trade.pnl) !== expectedPnlDirection) {
      warnings.push(`Row ${tradeIndex}: P&L sign doesn't match price movement for ${trade.direction} trade`);
    }
  }
...
export async function getWebhookPipelineStatus(): Promise<WebhookPipelineStatus> {
  ...
  const [orphanedExits] = await db.select({
...
export async function getPositionPipelineStatus(): Promise<PositionPipelineStatus> {
  ...
  const [openCount] = await db.select({
    count: sql<number>`COUNT(*)`,
  })
  .from(openPositions)
  ...
  const [orphanedCount] = await db.select({
    count: sql<number>`COUNT(*)`,
  })
  .from(openPositions)
  .where(and(
    eq(openPositions.status, 'closed'),
    isNull(openPositions.tradeId)
  ));
...
export async function validatePositionPipeline(): Promise<PipelineValidationResult> {
  ...
  if (status.stalePositions > 0) {
    checks.push({
      ...
      fixable: true,
      fixAction: 'Review and close stale positions or verify they are intentional',
    });
  } else {
    checks.push({
      name: 'Stale Positions',
 ...
 */
export async function validateTradeData(strategyId?: number): Promise<PipelineValidationResult> {
  const startTime = Date.now();
  const checks: PipelineCheck[] = [];
...
export async function validateAnalyticsPipeline(): Promise<PipelineValidationResult> {
  ...
  const strategiesWithTrades = await db.select({
  ...
  const strategiesWithTradesSet = new Set(strategiesWithTrades.map(s => s.strategyId));
  const strategiesWithoutTrades = allStrategies.filter(s => !strategiesWithTradesSet.has(s.id));
  ...
  if (dateRange?.minDate && dateRange?.maxDate) {
    const daysCovered = Math.round(
      (new Date(dateRange.maxDate).getTime() - new Date(dateRange.minDate).getTime()) / (1000 * 60 * 60 * 24)
    );
  ...
  if (recentTrade?.maxDate) {
    const daysSinceLastTrade = Math.round(
      (Date.now() - new Date(recentTrade.maxDate).getTime()) / (1000 * 60 * 60 * 24)
    );
    ...
    if (daysSinceLastTrade > 7) {
      checks.push({
        ...
        details: { lastTradeDate: recentTrade.maxDate, daysSince: daysSinceLastTrade },
      });
    } else {
      checks.push({
        name: 'Data Freshness',
...
export async function repairOrphanedPositions(): Promise<{
  ...
  const db = await getDb();
  if (!db) return { repaired: 0, failed: 0, errors: ['Database not available'] };
  ...
  for (const position of orphaned) {
    try {
      ...
      if (!strategy) {
        errors.push(`Position ${position.id}: Strategy not found for symbol ${position.strategySymbol}`);
        failed++;
        continue;
      }
...
export async function repairOrphanedExitWebhooks(): Promise<{
  ...
  for (const webhook of orphaned) {
    try {
      ...
      const [matchingTrade] = await db.select()
        .from(trades)
