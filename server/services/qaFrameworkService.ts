...
export function logError(
  ...
  if (severity === 'critical' || severity === 'error') {
    console.error(`[QA Framework] ${severity.toUpperCase()}: ${message}`, context);
  }
...
export async function generateDiagnosticReport(): Promise<DiagnosticReport> {
  ...
  if (db) {
    try {
      ...
      logError('database', 'error', 'Failed to generate diagnostic metrics', { error });
    }
  }
  ...
  if (systemHealth.database === 'degraded') {
    recommendations.push('Database response time is elevated. Consider checking connection pool settings.');
  }
  if (systemHealth.database === 'down') {
    recommendations.push('CRITICAL: Database connection failed. Check database server status immediately.');
  }
  if (metrics.webhookSuccessRate < 90) {
    recommendations.push(`Webhook success rate is ${metrics.webhookSuccessRate.toFixed(1)}%. Review recent webhook errors.`);
  }
  if (metrics.avgProcessingTimeMs > 1000) {
    recommendations.push(`Average webhook processing time is ${metrics.avgProcessingTimeMs.toFixed(0)}ms. Consider optimizing database queries.`);
  }
  if (metrics.openPositionsCount > 10) {
    recommendations.push(`${metrics.openPositionsCount} open positions detected. Verify all positions are intentional.`);
  }

  const unresolvedErrors = getRecentErrors({ unresolvedOnly: true, limit: 10 });
  if (unresolvedErrors.length > 0) {
    recommendations.push(`${unresolvedErrors.length} unresolved errors require attention.`);
  }
  ...
  try {
    switch (testName) {
      ...
      case 'position_tracking':
        ...
        await dbForPosition.execute(sql`SELECT COUNT(*) FROM open_positions LIMIT 1`);
        return {
          testName,
          passed: true,
          duration: Date.now() - startTime,
