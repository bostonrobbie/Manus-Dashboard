...
describe('compareStrategies - Null Strategy Handling', () => {
  ...
  it('should have strategy IDs starting from 9', async () => {
    const strategies = await db.getAllStrategies();
    const minId = Math.min(...strategies.map(s => s.id));
    
    // Verify the database has the expected strategy IDs
    expect(minId).toBe(9);
    expect(strategies.length).toBe(8); // 8 strategies total
  });
