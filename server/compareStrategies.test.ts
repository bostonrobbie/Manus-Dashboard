...
describe("compareStrategies - Null Strategy Handling", () => {
  ...
  it("should have 2 active NQ strategies", async () => {
    const strategies = await db.getAllStrategies();

    // Verify the database has the expected 2 NQ strategies
    expect(strategies.length).toBe(2); // NQ Trend and NQ Trend (Leveraged)
    expect(strategies.some(s => s.symbol === "NQTrend")).toBe(true);
    expect(strategies.some(s => s.symbol === "NQTrendLeveraged")).toBe(true);
  });
