...
describe('Backend API - Portfolio Overview', () => {
  ...
  it('should filter trades by 1Y time range', async () => {
    ...
    for (const trade of filteredTrades) {
      ...
      expect(exitDate.getTime()).toBeGreaterThanOrEqual(oneYearAgo.getTime());
      expect(exitDate.getTime()).toBeLessThanOrEqual(now.getTime());
    }
  });
  ...
  it('should filter trades by YTD time range', async () => {
    ...
    for (const trade of filteredTrades) {
      const exitDate = new Date(trade.exitDate);
      expect(exitDate.getTime()).toBeGreaterThanOrEqual(ytdStart.getTime());
      expect(exitDate.getTime()).toBeLessThanOrEqual(now.getTime());
    }
  });
  ...
  it('should calculate correlation matrix', async () => {
    ...
    for (const strategy of strategies) {
      ...
      if (trades.length > 0) {
        ...
        const startDate = rawEquity.length > 0 ? rawEquity[0]!.date : oneYearAgo;
        const filledEquity = analytics.forwardFillEquityCurve(rawEquity, startDate, now);
...
describe('Backend API - Performance Breakdown', () => {
  ...
  it('should calculate monthly performance breakdown', () => {
    const breakdown = analytics.calculatePerformanceByPeriod(allTrades, 'month');
...
describe('Backend API - Benchmark Data', () => {
  ...
  it('should calculate benchmark equity curve', async () => {
    ...
    const benchmarkEquity = analytics.calculateBenchmarkEquityCurve(benchmarkData, 100000);
