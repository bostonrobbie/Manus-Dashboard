...
export async function createOrder(params: {
  ...
  console.log(`[BrokerOrder] Created: ${internalOrderId} (${params.action} ${params.quantity} ${params.symbol})`);
 ...
 */
export async function markSubmitted(internalOrderId: string, brokerOrderId?: string): Promise<void> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  console.log(`[BrokerOrder] Submitted: ${internalOrderId} -> ${brokerOrderId}`);
}
...
export async function updateOrderStatus(
  ...
  if (details?.filledQuantity !== undefined) updates.filledQuantity = details.filledQuantity;
  if (details?.avgFillPrice !== undefined) updates.avgFillPrice = details.avgFillPrice;
  if (details?.commission !== undefined) updates.commission = details.commission;
...
export async function markFilled(
  ...
  if (orders.length > 0 && orders[0].tradeId) {
    ...
    console.log(`[BrokerOrder] Updated trade ${orders[0].tradeId} with fill price ${fillDetails.avgFillPrice}`);
  }

  console.log(`[BrokerOrder] Filled: ${internalOrderId} @ ${fillDetails.avgFillPrice}`);
}

/**
 * Mark order as rejected
 */
export async function markRejected(internalOrderId: string, reason: string): Promise<void> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
 ...
 */
export async function getOrder(internalOrderId: string): Promise<BrokerOrderRecord | null> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getPendingOrders(): Promise<BrokerOrderRecord[]> {
  ...
  const pendingStatuses: OrderStatus[] = ["submitted", "acknowledged", "working", "partially_filled"];
 ...
 */
export async function linkOrderToTrade(internalOrderId: string, tradeId: number): Promise<void> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getOrderStats(): Promise<{
  ...
  for (const r of results) {
    if (["pending", "submitted", "acknowledged", "working", "partially_filled"].includes(r.status)) {
      stats.pending++;
    } else if (r.status === "filled") {
      stats.filled++;
    } else if (["rejected", "cancelled", "expired", "error"].includes(r.status)) {
      stats.rejected++;
    }
  }
