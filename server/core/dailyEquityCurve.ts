...
import { getTradingCalendar, isMarketOpen, TradingCalendar } from './tradingCalendar';
 ...
 */
function aggregateTradesByDay(trades: Trade[]): Map<string, { pnl: number; count: number }> {
  ...
  for (const trade of trades) {
    ...
    existing.pnl += trade.pnl / 100; // Convert cents to dollars
    existing.count += 1;
...
export function calculateDailyEquityCurve(
  ...
  const dailyPnLMap = aggregateTradesByDay(sortedTrades);
...
export function calculateDailySharpeRatio(
  ...
  }, 0) / (dailyReturns.length - 1); // Use N-1 for sample std dev
...
export function calculateDailySortinoRatio(
  ...
  const sortino = ((meanReturn - dailyRiskFreeRate) / downsideStdDev) * Math.sqrt(252);
 ...
 */
export function calculateDailyMaxDrawdown(
  dailyCurve: DailyEquityPoint[]
): { percentage: number; dollars: number; peakDate: Date | null; troughDate: Date | null } {
