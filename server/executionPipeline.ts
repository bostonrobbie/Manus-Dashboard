...
class ExecutionPipeline {
    ...
    try {
      ...
      const subscriptions = await db
        ...
        .where(
          and(
            ...
            eq(userSubscriptions.autoExecuteEnabled, true)
          )
        );
      
      if (subscriptions.length === 0) {
        console.log(`[ExecutionPipeline] No active subscriptions for strategy ${strategyId}`);
        return {
          success: true,
          signalsCreated: 0,
          usersNotified: 0,
      ...
      for (const sub of subscriptions) {
        try {
          ...
          const multiplier = sub.quantityMultiplier ? parseFloat(String(sub.quantityMultiplier)) : 1;
          if (multiplier !== 1) {
  ...
  async updateOrderStatus(
    orderId: number,
    status: 'pending' | 'submitted' | 'filled' | 'partial' | 'cancelled' | 'rejected' | 'error',
    details?: {
    ...
    try {
      ...
      await db.update(executionLogs)
        .set({
          ...
          ...(status === 'filled' || status === 'partial' ? { filledAt: new Date() } : {}),
        })
...
export async function expirePendingSignals(): Promise<number> {
  try {
    ...
    const result = await db.update(userSignals)
      .set({
        action: 'expired',
        actionTakenAt: new Date(),
      })
