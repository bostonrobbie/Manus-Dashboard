...
export async function addToRetryQueue(
  ...
  console.log(`[RetryQueue] Added webhook ${correlationId} to retry queue, next retry at ${nextRetryAt.toISOString()}`);
 ...
 */
export async function getPendingRetries(limit: number = 10): Promise<Array<{
  id: number;
  originalPayload: string;
  correlationId: string;
  strategySymbol: string | null;
 ...
 */
export async function scheduleNextRetry(id: number, retryCount: number, error: string): Promise<void> {
  const db = await getDb();
  if (!db) return;
  ...
  console.log(`[RetryQueue] Item ${id} scheduled for retry #${retryCount + 1} at ${nextRetryAt.toISOString()}`);
}
  ...
  try {
    ...
    if (result.success) {
      ...
      if (item.retryCount + 1 >= item.maxRetries) {
        ...
        await scheduleNextRetry(item.id, item.retryCount, result.error || 'Unknown error');
      }
    }
  } catch (error) {
...
export async function cancelRetry(id: number): Promise<boolean> {
  ...
  const result = await db.update(webhookRetryQueue)
    .set({ status: 'cancelled' })
    .where(and(
      eq(webhookRetryQueue.id, id),
      eq(webhookRetryQueue.status, 'pending')
    ));
...
export async function cleanupOldItems(olderThanDays: number = 7): Promise<number> {
  ...
  const result = await db.delete(webhookRetryQueue)
    .where(and(
      sql`${webhookRetryQueue.status} IN ('completed', 'failed', 'cancelled')`,
      lte(webhookRetryQueue.completedAt, cutoffDate)
    ));
