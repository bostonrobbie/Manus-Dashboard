...
async function processBatch(batchKey: string): Promise<void> {
  ...
  try {
    ...
    console.log(`[BatchProcessor] Processed batch ${batch.batchId}: ${batch.signals.length} signals -> net ${netResult.direction} ${netResult.quantity} @ ${netResult.avgPrice}`);
    ...
    console.error(`[BatchProcessor] Failed to process batch ${batch.batchId}:`, error);
  }
}
 ...
 */
function calculateNetPosition(signals: Array<{
  action: string;
  direction: string;
  price: number;
  quantity: number;
...
export async function getBatchStats(): Promise<{
  ...
  const db = await getDb();
  if (!db) {
    return { collecting: 0, processing: 0, completed: 0, failed: 0, pendingInMemory: pendingBatches.size };
  }
  ...
  const result = { collecting: 0, processing: 0, completed: 0, failed: 0, pendingInMemory: pendingBatches.size };
