...
async function main() {
  ...
  const [existingUser] = await db.select().from(schema.users).where(eq(schema.users.openId, "demo-user"));
  let userId = existingUser?.id;
  if (!userId) {
    await db.insert(schema.users).values({ openId: "demo-user", email: "demo@example.com", name: "Demo User" });
    userId = (await db.select().from(schema.users).where(eq(schema.users.openId, "demo-user")))[0]?.id;
  }
  ...
  console.log(`Seeded demo data for user ${userId} with trades and benchmarks`);
}
