...
describe('Webhook Service', () => {
  describe('mapSymbolToStrategy', () => {
    it('should map ES symbols to ESTrend', () => {
    ...
    it('should map BTC symbols to BTCTrend', () => {
      expect(mapSymbolToStrategy('BTC')).toBe('BTCTrend');
      expect(mapSymbolToStrategy('BTCUSD')).toBe('BTCTrend');
      expect(mapSymbolToStrategy('BTC1!')).toBe('BTCTrend');
    });

    it('should map CL symbols to CLTrend', () => {
      expect(mapSymbolToStrategy('CL')).toBe('CLTrend');
      expect(mapSymbolToStrategy('CL1!')).toBe('CLTrend');
    });

    it('should map GC symbols to GCTrend', () => {
      expect(mapSymbolToStrategy('GC')).toBe('GCTrend');
      expect(mapSymbolToStrategy('GC1!')).toBe('GCTrend');
    });

    it('should map YM symbols to YMORB', () => {
      expect(mapSymbolToStrategy('YM')).toBe('YMORB');
      expect(mapSymbolToStrategy('YM1!')).toBe('YMORB');
    });
  ...
  describe('validatePayload', () => {
    describe('TradingView format (symbol, date, data, price)', () => {
      ...
      it('should validate an exit signal', () => {
        ...
        const result = validatePayload(payload);
        expect(result.strategySymbol).toBe('BTCTrend');
    ...
    describe('Error handling', () => {
      ...
      it('should throw error for missing symbol', () => {
        ...
        expect(() => validatePayload(payload)).toThrow('Missing or invalid "symbol" or "strategy" field');
      });
      ...
      it('should throw error for missing action/data', () => {
        ...
        expect(() => validatePayload(payload)).toThrow('Missing or invalid "data" or "action" field');
      });
  ...
  describe('Action normalization', () => {
    const testCases = [
      ...
      { input: 'SHORT', expectedAction: 'sell', expectedDirection: 'Short' },
      { input: 'entry_short', expectedAction: 'sell', expectedDirection: 'Short' },
      ...
      { input: 'exit_short', expectedAction: 'exit', expectedDirection: 'Short' },
    ];
  ...
  describe('Real-world payload examples', () => {
    it('should handle TradingView webhook with placeholders resolved', () => {
      ...
      expect(result.action).toBe('buy');
      expect(result.direction).toBe('Long');
      expect(result.price).toBe(42150.50);
    });
