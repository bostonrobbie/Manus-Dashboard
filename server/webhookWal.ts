...
// WAL entry status
export type WalStatus = "received" | "processing" | "completed" | "failed" | "retrying";
 ...
 */
export async function markCompleted(walId: string, webhookLogId?: number): Promise<void> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db
    .update(webhookWal)
 ...
 */
export async function markFailed(walId: string, errorMessage: string): Promise<void> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db
    .update(webhookWal)
 ...
 */
export async function markForRetry(walId: string, errorMessage: string): Promise<void> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db
    .update(webhookWal)
 ...
 */
export async function getStuckWebhooks(maxAge: number = 5 * 60 * 1000): Promise<WalEntry[]> {
  ...
  const results = await db
    ...
    .where(
      and(
        ...
        lt(webhookWal.lastAttemptAt, cutoff)
      )
    );
 ...
 */
export async function getRetryableWebhooks(maxAttempts: number = 5): Promise<WalEntry[]> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const results = await db
    .select()
    ...
    .where(
      and(
        ...
        lt(webhookWal.attempts, maxAttempts)
      )
    );
  ...
  const result = await db
    ...
    .where(
      and(
        ...
        lt(webhookWal.completedAt, cutoff)
      )
    );

  const deletedCount = (result as unknown as { affectedRows?: number })?.affectedRows ?? 0;
...
export async function recoverStuckWebhooks(): Promise<number> {
  ...
  console.log(`[WAL] Found ${stuck.length} stuck webhooks, marking for retry...`);
