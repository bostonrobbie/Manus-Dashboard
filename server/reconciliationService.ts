...
import { reconciliationLogs, openPositions, positionAdjustments } from "../drizzle/schema";
...
export async function runReconciliation(
  ...
  for (const brokerPos of brokerPositions) {
    ...
    if (!dbPos) {
      ...
      // Check for significant price mismatch (>5% difference)
        ...
        const priceDiff = Math.abs(dbPos.entryPrice - brokerPos.avgPrice) / dbPos.entryPrice;
        if (priceDiff > 0.05) {
          discrepancies.push({
            type: "price_mismatch",
            symbol: brokerPos.symbol,
  ...
  console.log(`[Reconciliation] ${reconciliationId}: ${result.matchedCount} matched, ${result.discrepancyCount} discrepancies`);
 ...
 */
export async function getUnresolvedDiscrepancies(broker?: string): Promise<Array<{
  id: number;
  reconciliationId: string;
  runAt: Date;
  broker: string;
...
export async function resolveDiscrepancy(
  ...
  console.log(`[Reconciliation] Resolved discrepancy ${discrepancyId}: ${resolution.action}`);
}
...
export async function forceClosePosition(
  ...
  console.log(`[Reconciliation] Force closed position ${positionId}: ${reason}`);
}
...
export async function syncPositionFromBroker(
  ...
  console.log(`[Reconciliation] Synced position for ${strategySymbol} from broker`);
}

/**
 * Get position adjustment history
 */
export async function getAdjustmentHistory(strategySymbol?: string): Promise<Array<{
  id: number;
  strategySymbol: string;
